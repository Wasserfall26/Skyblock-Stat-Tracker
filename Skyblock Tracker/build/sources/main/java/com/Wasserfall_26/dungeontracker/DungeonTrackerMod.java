...
public class DungeonTrackerMod {
    ...
    public void init(FMLInitializationEvent event) {
        ...
        System.out.println("[SkyBlock Tracker] Use MIDDLE-CLICK + ALT on player names to open tracker");
    ...
    public void onMouseInput(GuiScreenEvent.MouseInputEvent.Pre event) {
        ...
        if (Mouse.getEventButton() == 2 && Mouse.getEventButtonState()) {
            ...
            if (altHeld && Minecraft.getMinecraft().currentScreen instanceof GuiChat) {
                GuiChat chatGui = (GuiChat) Minecraft.getMinecraft().currentScreen;

                // Get the chat component at mouse position using reflection (1.8.9 compatibility)
                IChatComponent hoveredComponent = getChatComponentAt(chatGui, Mouse.getX(), Mouse.getY());

                if (hoveredComponent != null) {
                    String text = hoveredComponent.getUnformattedText();

                    // Split and search for player names
                    String[] words = text.split("[\\s\\[\\]():><!]+");

                    for (String word : words) {
                        if (word.matches("[A-Za-z0-9_]{3,16}") && !isCommonWord(word)) {
                            pendingCommands.add(word);
                            System.out.println("[SkyBlock Tracker] Middle-clicked on player: " + word);
                            break;
                        }
                    }
                }
    ...
    /**
     * Get chat component at mouse position (1.8.9 compatible)
     */
    private IChatComponent getChatComponentAt(GuiChat chatGui, int mouseX, int mouseY) {
        try {
            // Use reflection to access the func_146236_a method (getChatComponent in 1.9+)
            // Method signature: func_146236_a(int mouseX, int mouseY)
            java.lang.reflect.Method method = GuiChat.class.getDeclaredMethod("func_146236_a", int.class, int.class);
            method.setAccessible(true);
            return (IChatComponent) method.invoke(chatGui, mouseX, mouseY);
        } catch (Exception e) {
            System.err.println("[SkyBlock Tracker] Failed to get chat component at mouse position:");
            e.printStackTrace();
            return null;
        }
    }
     ...
     * NOTE: Left-click still works but only triggers with ALT held
     * Middle-click is the new primary method
     */
